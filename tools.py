...
# =====================================================================
# Parent panel — everything else nests under this one.
# =====================================================================
class KINGPIN_PT_main(Panel):
    bl_space_type = 'PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_context = 'scene'
    bl_label = '.MD2/.MDX Model Tools'
    bl_options = set()

    def draw(self, context):
        # Header-only parent; sub-panels carry the content.
        # Import/export live under File > Import / File > Export.
        pass


# -=| GUI |=- Grid (modeling helper)
class KINGPIN_PT_grid(Panel):
    bl_space_type = 'PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_context = 'scene'
    bl_parent_id = 'KINGPIN_PT_main'
    bl_label = 'MD2 vertex grid'
...
# -=| GUI |=- Smooth (modeling helper)
class KINGPIN_PT_smooth(Panel):
    bl_space_type = 'PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_context = 'scene'
    bl_parent_id = 'KINGPIN_PT_main'
    bl_label = 'Smooth animation wobble'
    bl_options = {'DEFAULT_CLOSED'}
...
# -=| GUI |=- Retarget (animation helper)
class KINGPIN_PT_retarget(Panel):
    bl_space_type = 'PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_context = 'scene'
    bl_parent_id = 'KINGPIN_PT_main'
...
class KINGPIN_UI_BUTTON_DRIVER(Operator):
    ...
    def execute(self, context):
        ...
        for i, obj in enumerate(sel_objs):
            ...
            # calc_normals_split was removed in Blender 4.1; normals are
            # now computed automatically when mesh.update() runs.
    ...
    def execute(self, context):
        ...
        for oIdx, obj in enumerate(sel_objs):
            ...
            if anim is None or anim.action is None:
                ...
                return {'FINISHED'}
            # Resolve fcurves once per action; works on 4.4+ slotted actions.
            anim_fcurves = fcurves_get(anim.action)
            # loop through vertex array
            for i in range(len(obj.data.vertices)):
                for j in range(3):
                    fcu = anim_fcurves[i*3+j] # x0, y0, z0, x1, y1, z1
...
classes = (
    ...
    # Parent must register before sub-panels.
    KINGPIN_PT_main,
    KINGPIN_PT_grid,
    KINGPIN_PT_smooth,
    KINGPIN_PT_retarget,
