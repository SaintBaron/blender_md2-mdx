    ...
    def makeObject(self):
        ...
        if self.numFrames > 1 and self.ui_opt_anim:
            ...
            if sk_type == 1:
                ...
                new_action = bpy.data.actions.new("KP_Anim")
                # Use slot-aware assignment so Blender 4.4+ binds the
                # mesh data-block to a compatible slot.
                assign_action_with_slot(obj_dat, new_action,
                                        slot_id_type='MESH')
                obj_act = obj_dat.animation_data.action

                for v in obj_dat.vertices:
                    # fcurves_new() chooses the legacy or channelbag API
                    # depending on Blender version. Mesh vertex co lives
                    # on the mesh data-block, so the slot type is 'MESH'.
                    fcurves = [fcurves_new(
                        obj_act,
                        "vertices[%d].co" % v.index,
                        index=i,
                        group_name="V%s" % v.index,
                        slot_id_type='MESH') for i in range(3)]
